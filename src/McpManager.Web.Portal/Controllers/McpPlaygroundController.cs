...
public class McpPlaygroundController : BaseController
{
    ...
    public IActionResult Index()
    {
        ...
        var servers = _mcpServerRepository
            .GetAll()
            .Where(s => s.IsActive)
            .OrderBy(s => s.Name)
    ...
    public async Task<IActionResult> GetTools(Guid serverId)
    {
        ...
        var tools = await _mcpToolRepository
            .GetByServer(server)
            .OrderBy(t => t.Name)
        ...
        try
        {
            ...
            foreach (var prop in properties.Properties())
            {
                ...
                if (propValue["default"] != null)
                {
                    ...
                    field.Default =
                        defaultVal.Type == JTokenType.Object || defaultVal.Type == JTokenType.Array
                ...
                if (
                    propValue["minimum"] != null
                    && int.TryParse(propValue["minimum"].ToString(), out var min)
                )
                {
                    field.Minimum = min;
                }
                ...
                if (
                    propValue["maximum"] != null
                    && int.TryParse(propValue["maximum"].ToString(), out var max)
                )
                {
                    field.Maximum = max;
                }
