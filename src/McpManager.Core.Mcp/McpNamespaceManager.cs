...
public class McpNamespaceManager
{
    ...
    {
        var existing = await _namespaceServerRepository
            .GetByNamespace(ns)
        ...
        var nsServer = new McpNamespaceServer { McpNamespaceId = ns.Id, McpServerId = server.Id };
        ...
        _logger.LogInformation(
            "Added server {ServerName} to namespace {NsName}",
            server.Name,
            ns.Name
        );
        return nsServer;
    ...
    public async Task UpdateToolOverride(
        McpNamespaceTool nsTool,
        string nameOverride,
        string descriptionOverride
    )
    {
        nsTool.NameOverride = string.IsNullOrWhiteSpace(nameOverride) ? null : nameOverride.Trim();
        nsTool.DescriptionOverride = string.IsNullOrWhiteSpace(descriptionOverride)
            ? null
    ...
    public async Task SyncToolsForServer(McpNamespaceServer nsServer)
    {
        ...
        var existingNsTools = await _namespaceToolRepository
        ...
        foreach (var tool in serverTools)
        {
            ...
            {
                _namespaceToolRepository.Add(
                    new McpNamespaceTool { McpNamespaceServerId = nsServer.Id, McpToolId = tool.Id }
                );
            }
        }
    ...
    {
        var existing = await _namespaceRepository
            .GetBySlug(ns.Slug)
