...
public class ApiKeyManager {
    ...
    public async Task<ApiKey> Create(ApiKey apiKey) {
        ...
        _logger.LogInformation("Created API key {ApiKeyName} with ID {ApiKeyId}", apiKey.Name, apiKey.Id);
        return apiKey;
    }
    ...
    public async Task Delete(ApiKey apiKey) {
        ...
        await _apiKeyRepository.SaveChanges();
        _logger.LogInformation("Deleted API key {ApiKeyName} with ID {ApiKeyId}", apiKey.Name, apiKey.Id);
    }
    ...
    public async Task ToggleActive(ApiKey apiKey) {
        ...
        await _apiKeyRepository.SaveChanges();
        _logger.LogInformation("Toggled API key {ApiKeyName} active={IsActive}", apiKey.Name, apiKey.IsActive);
    }

    private static string GenerateApiKey() {
        var bytes = RandomNumberGenerator.GetBytes(32);
        return "mcpm_" + Convert.ToBase64String(bytes).Replace("+", "").Replace("/", "").Replace("=", "");
    }
}
