...
public class McpServerManager {
    ...
    public async Task<McpServer> Create(McpServer server) {
        ...
        _logger.LogInformation("Created MCP server {ServerName} with ID {ServerId}", server.Name, server.Id);
        return server;
    }
    ...
    public async Task Update(McpServer server) {
        ...
        ValidateServer(server);
        await _serverRepository.SaveChanges();
        _logger.LogInformation("Updated MCP server {ServerName} with ID {ServerId}", server.Name, server.Id);
    }
    ...
    public async Task Delete(McpServer server) {
        ...
        await _serverRepository.SaveChanges();
        _logger.LogInformation("Deleted MCP server {ServerName} with ID {ServerId}", server.Name, server.Id);
    }
    ...
    public async Task<bool> CheckHealth(McpServer server) {
        ...
        try {
            if (server.TransportType == McpTransportType.OpenApi) {
                ...
                if (!healthy) throw new Exception("Server returned 5xx status");
            } else {
            ...
            var endpoint = server.TransportType == McpTransportType.Stdio ? server.Command : server.Uri;
    ...
    public async Task<SyncResult> SyncTools(McpServer server) {
        ...
        try {
            ...
            if (server.TransportType == McpTransportType.OpenApi) {
                ...
                toolEntries = operations.Select(op => new ToolSyncEntry {
                    Name = op.Name,
                ...
                toolEntries = remoteTools.Select(rt => new ToolSyncEntry {
                    Name = rt.Name,
                    Description = rt.Description ?? "",
                    InputSchema = rt.JsonSchema.ValueKind != JsonValueKind.Undefined
            ...
            _logBuffer.Add(new LogEntry {
                Level = "Info", Message = $"Sync completed: {result.ToolsAdded} added, {result.ToolsUpdated} updated, {result.ToolsRemoved} removed",
                ServerId = server.Id, ServerName = server.Name
            });
            await CreateServerLog(server, McpServerLogLevel.Info,
            ...
            _logger.LogInformation(
                ...
                server.Name, result.ToolsAdded, result.ToolsUpdated, result.ToolsRemoved);
        } catch (Exception ex) {
            var endpoint = server.TransportType == McpTransportType.Stdio ? server.Command : server.Uri;
            var errorMessage = $"Failed to connect to {endpoint}: {ex.Message}";
    ...
    public async Task<ToolExecutionResult> CallTool(McpServer server, string toolName, Dictionary<string, object> arguments, string apiKeyName = null, Guid? namespaceId = null) {
        ArgumentNullException.ThrowIfNull(server);
        ...
        try {
            ...
            _logBuffer.Add(new LogEntry {
                Level = "Info", Message = $"Tool '{toolName}' executed successfully ({stopwatch.ElapsedMilliseconds}ms)",
                ServerId = server.Id, ServerName = server.Name, ToolName = toolName
            });
        } catch (Exception ex) {
            result.Error = ex.Message;
            await CreateServerLog(server, McpServerLogLevel.Error,
                $"Tool execution failed for {toolName}: {ex.Message}");
            _logBuffer.Add(new LogEntry {
                Level = "Error", Message = $"Tool '{toolName}' failed: {ex.Message}",
                ServerId = server.Id, ServerName = server.Name, ToolName = toolName
            });
            _logger.LogWarning(ex, "Tool execution failed for {ToolName} on server {ServerName}", toolName, server.Name);
        }
    ...
    private async Task RecordToolRequest(McpServer server, string toolName, Dictionary<string, object> arguments, ToolExecutionResult result, string apiKeyName, Guid? namespaceId = null) {
        try {
            var tool = await _toolRepository.GetByName(server, toolName).FirstOrDefaultAsync();
            if (tool == null) return;
    ...
    private async Task CreateServerLog(McpServer server, McpServerLogLevel level, string message) {
        try {
            ...
            _logger.LogWarning(ex, "Failed to create server log entry for {ServerName}", server.Name);
        }
    }
    ...
    private void MergeTools(McpServer server, List<ToolSyncEntry> toolEntries, SyncResult result) {
        ...
        foreach (var entry in toolEntries) {
            if (existingToolsByName.TryGetValue(entry.Name, out var existingTool)) {
                var needsUpdate = existingTool.Description != entry.Description ||
    ...
    public async Task UpdateToolCustomization(McpTool tool, string customDescription, List<(string Name, string CustomDescription)> argumentOverrides) {
        ArgumentNullException.ThrowIfNull(tool);

        tool.CustomDescription = string.IsNullOrWhiteSpace(customDescription) ? null : customDescription.Trim();
        ...
        if (argumentOverrides?.Any(a => !string.IsNullOrWhiteSpace(a.CustomDescription)) == true) {
            try {
                ...
                if (properties != null) {
                    foreach (var arg in argumentOverrides.Where(a => !string.IsNullOrWhiteSpace(a.CustomDescription))) {
    ...
    public async Task ApplyToolCustomizations(McpServer server, List<(string ToolName, string OriginalDescription, string CustomDescription)> customizations) {
        ArgumentNullException.ThrowIfNull(server);
        if (customizations == null) return;

        var hasChanges = false;
        foreach (var customization in customizations) {
            if (string.IsNullOrWhiteSpace(customization.CustomDescription) ||
                customization.CustomDescription.Trim() == customization.OriginalDescription?.Trim()) {
                continue;
            }
    ...
    private void ValidateServer(McpServer server) {
        ...
        if (server.TransportType == McpTransportType.OpenApi) {
            ...
            if (!Uri.TryCreate(server.Uri, UriKind.Absolute, out var openApiUri) ||
                (openApiUri.Scheme != Uri.UriSchemeHttp && openApiUri.Scheme != Uri.UriSchemeHttps)) {
            ...
            if (string.IsNullOrWhiteSpace(server.OpenApiSpecification)) {
                throw new ApplicationException("OpenAPI specification is required", "OpenApiSpecification");
            }
            ...
            if (!Uri.TryCreate(server.Uri, UriKind.Absolute, out var uri) ||
                (uri.Scheme != Uri.UriSchemeHttp && uri.Scheme != Uri.UriSchemeHttps)) {
                throw new ApplicationException("Server URI must be a valid HTTP or HTTPS URL", "Uri");
            }

            switch (server.Auth.Type) {
                case AuthType.Basic:
                    if (string.IsNullOrWhiteSpace(server.Auth.Username)) {
                        throw new ApplicationException("Username is required for Basic authentication", "Auth.Username");
                    }
                    break;

                case AuthType.Bearer:
                    if (string.IsNullOrWhiteSpace(server.Auth.Token)) {
                        throw new ApplicationException("Token is required for Bearer authentication", "Auth.Token");
                    }
                    break;

                case AuthType.ApiKey:
                    if (string.IsNullOrWhiteSpace(server.Auth.ApiKeyName)) {
                        throw new ApplicationException("API key name is required for ApiKey authentication", "Auth.ApiKeyName");
                    }
                    if (string.IsNullOrWhiteSpace(server.Auth.ApiKeyValue)) {
                        throw new ApplicationException("API key value is required for ApiKey authentication", "Auth.ApiKeyValue");
                    }
                    break;
            }
        } else {
            if (string.IsNullOrWhiteSpace(server.Command)) {
                throw new ApplicationException("Command is required for Stdio transport", "Command");
            }
        }
    }
}
