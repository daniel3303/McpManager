...
public class UsersController : BaseController {
    ...
    public UsersController(UserManager<User> userManager, UserRepository userRepository, IFlashMessage flashMessage) {
    ...
    public IActionResult Create() {
        ...
        return View(new UserForCreateDto { Claims = BuildClaimCheckboxItems(new HashSet<string>()) });
    }
    ...
    private static List<ClaimCheckboxItem> BuildClaimCheckboxItems(HashSet<string> enabledClaims) {
        return ClaimStore.ClaimGroups()
            .SelectMany(g => g.Claims.Select(c => new ClaimCheckboxItem {
                Type = c.Type,
                Name = c.Name,
    ...
    private async Task UpdateUserClaims(User user, List<string> enabledClaims) {
        ...
        var claimsToRemove = currentClaims.Where(c => !enabledClaims.Contains(c) && allClaimTypes.Contains(c)).ToList();
        ...
        var claimsToAdd = enabledClaims.Where(c => !currentClaims.Contains(c) && allClaimTypes.Contains(c)).ToList();
