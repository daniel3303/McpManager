...
public static class GridColumnsOfExtensions
{
    ...
    public static void AddActivating<T>(this IGridColumnsOf<T> gridColumn, string title = "Status")
        where T : IActivable
    {
        gridColumn
            .Add(m => m.IsActive)
    ...
    public static void AddActions<T>(
        this IGridColumnsOf<T> gridColumn,
        Action<IActionBuilder<T>> action,
        string title = ""
    )
    {
        ...
        gridColumn
            ...
            .RenderedAs(m =>
            {
                var builder = new ActionBuilder<T>(
                    m,
                    httpContext,
                    httpContext.RequestServices.GetService<LinkGenerator>(),
                    httpContext.GetController(),
                    gridColumn
                );
                action(builder);
            ...
            .Titled(title)
            .Encoded(false)
            .CssClasses = "p-2 w-1 white-space-nowrap align-middle";
    }
}
...
public interface IActionBuilder<TEntity>
{
    ...
    public IActionBuilder<TEntity> AddShow(
        string action = null,
    ...
    public IActionBuilder<TEntity> AddInfo(
        string action = null,
        string controller = null,
        object routeValues = null,
        string text = "Info"
    ...
    public IActionBuilder<TEntity> AddDelete(
    ...
    public IActionBuilder<TEntity> AddCustomHtml(
        Func<TEntity, string> action,
        bool verification = true,
        Func<TEntity, bool> when = null
    );
...
public class ActionBuilder<TEntity> : IActionBuilder<TEntity>
{
    ...
    private static readonly string SvgInfo = HeroIcons.Render(
        "information-circle",
        HeroIcons.IconStyle.Outline,
        4
    );
    private static readonly string SvgTrash = HeroIcons.Render(
        "trash",
        HeroIcons.IconStyle.Outline,
        4
    );
    ...
    private static string GetIcon(string name) =>
        HeroIcons.Render(name, HeroIcons.IconStyle.Outline, 4);
    ...
    public ActionBuilder(
        ...
        string controller,
        IGridColumnsOf<TEntity> gridColumn
    )
    {
        Model = model;
        HttpContext = context;
    ...
    )
    {
        ...
        Html +=
            $"<a href=\"{link}\" class=\"grid-action-btn\" data-tooltip=\"{text}\" {(targetBlank ? "target=\"_blank\"" : "")}>{SvgEye}</a>";
        return this;
    }
    ...
    )
    {
        ...
        var link = LinkGenerator.GetPathByAction(HttpContext, action, controller, routeValues);
        Html +=
            $"<a href=\"{link}\" class=\"grid-action-btn\" data-tooltip=\"{text}\">{SvgInfo}</a>";
        return this;
    }
    ...
    )
    {
        ...
        var antiforgeryInput =
            $@"<input type=""hidden"" name=""{tokenSet.FormFieldName}"" value=""{tokenSet.RequestToken}"" />";

        Html +=
            $@"<form method=""post"" class=""form-confirm inline"" data-message=""{confirm}"" action=""{link}"">
            {antiforgeryInput}
    ...
    )
    {
        ...
        {
            Html +=
                $"<button type=\"button\" class=\"grid-action-btn {cssClass}\" data-tooltip=\"{text}\"{dataAttrs}>{iconHtml}</button>";
            return this;
        }
        ...
        if (isPost)
        {
            ...
            Html +=
                $@"<form method=""post"" class=""{confirmClass} inline"" {confirmAttr} action=""{link}"">
                {antiforgeryInput}
        ...
        {
            Html +=
                $"<a href=\"{link}\" class=\"grid-action-btn {cssClass}\" data-tooltip=\"{text}\"{dataAttrs}>{iconHtml}</a>";
        }
