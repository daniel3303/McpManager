...
public class McpNamespaceProxyServer {
    ...
    public async ValueTask<ListToolsResult> ListToolsHandler(
        ...
        var tools = nsTools.Select(nst => new Tool {
            Name = nst.NameOverride ?? nst.McpTool.Name,
            Description = nst.DescriptionOverride ?? nst.McpTool.CustomDescription ?? nst.McpTool.Description,
            InputSchema = McpProxyHelpers.ParseInputSchema(nst.McpTool.CustomInputSchema ?? nst.McpTool.InputSchema)
        }).ToList();
    ...
    public async ValueTask<CallToolResult> CallToolHandler(
        ...
        var nsTool = nsTools.FirstOrDefault(t =>
            (t.NameOverride ?? t.McpTool.Name) == toolName);

        if (nsTool == null) {
            throw new McpProtocolException($"Tool '{toolName}' not found in namespace '{ns.Slug}'", McpErrorCode.InvalidRequest);
        }
        ...
        _logger.LogInformation("Calling tool {ToolName} (original: {OriginalName}) on server {ServerName} via namespace {NsSlug}",
            toolName, originalToolName, server.Name, ns.Slug);

        var result = await _mcpServerManager.CallTool(server, originalToolName, arguments, apiKeyName, ns.Id);
    ...
    private async Task<List<McpNamespaceTool>> GetEnabledNamespaceTools(McpNamespace ns, CancellationToken cancellationToken) {
        return await _namespaceToolRepository.GetAll()
            .Where(t => t.McpNamespaceServer.McpNamespaceId == ns.Id
