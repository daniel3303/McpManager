...
class MvcGridDateOnlyFilter extends MvcGridDateFilter {
    constructor(column) {
        super(column);
...
class MvcGridTimeOnlyFilter extends MvcGridDateFilter {
    constructor(column) {
        super(column);
...
const selector = ".mvc-grid";

// Initialize existing grids on page load
...
// Mutation observer to apply mvc grid to new elements
    ...
    for (let i = 0; i < mutations.length; ++i) {
        ...
        for (let j = 0; j < mutations[i].addedNodes.length; ++j) {
            ...
            if (element.matches && element.matches(selector)) {
                CreateMVCGrid($(mutations[i].addedNodes[j]));
            } else if (element.querySelectorAll) {
                const grids = element.querySelectorAll(selector);
