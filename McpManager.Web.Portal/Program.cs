...
builder.Services.AddMcpServer(options => {
    ...
    options.ServerInstructions = "MCP proxy server that aggregates tools from multiple upstream MCP servers.";
    ...
    httpOptions.ConfigureSessionOptions = (httpContext, mcpOptions, _) => {
        ...
        if (!string.IsNullOrEmpty(slug)) {
            var nsProxy = httpContext.RequestServices.GetRequiredService<McpNamespaceProxyServer>();
...
builder.Services.AddIdentity<User, McpManager.Core.Data.Models.Identity.Role>(options => {
...
builder.Services.AddRateLimiter(options => {
    ...
    options.AddPolicy("NamespaceRateLimit", httpContext => {
        var slug = httpContext.Request.RouteValues["slug"] as string;
        if (string.IsNullOrEmpty(slug)) {
...
builder.Services.ConfigureApplicationCookie(options => {
    ...
    options.Events.OnRedirectToLogin = ctx => {
        ...
        var linkGenerator = ctx.Request.HttpContext.RequestServices.GetRequiredService<LinkGenerator>();
